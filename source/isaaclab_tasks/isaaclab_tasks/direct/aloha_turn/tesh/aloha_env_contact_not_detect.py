...
class WheeledRobotEnvCfg(DirectRLEnvCfg):
    ...
    scene: InteractiveSceneCfg = InteractiveSceneCfg(num_envs=512, env_spacing=9, replicate_physics=True)
    ...
    contact_sensor = ContactSensorCfg(
        prim_path="/World/envs/env_.*/Robot/.*(box2_Link|base_link)",  # Только box2_Link и base_link
        update_period=0.0,
        debug_vis=False,
    )
...
class WheeledRobotEnv(DirectRLEnv):
    ...
    def __init__(self, cfg: WheeledRobotEnvCfg, render_mode: str | None = None, **kwargs):
        ...
        self._episode_sums = {
            ...
            for key in ["goal_reached_reward", "out_of_bounds_penalty", "contact_penalty"]
        }
    ...
    def _get_rewards(self) -> torch.Tensor:
        ...
        try:
            ...
            # Вывод сред с контактом
            # if has_contact.any():
            contact_envs = torch.where(has_contact)[0].cpu().numpy()
            print(f"Environments with contact: {contact_envs.tolist()}")
            contact_penalty_scale = -100.0
            contact_penalty = has_contact.float() * contact_penalty_scale
        ...
        reward = (-1 + goal_reached_reward + out_of_bounds_penalty + contact_penalty)
    ...
    def _get_dones(self) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        root_pos_w = self._robot.data.root_pos_w[:, :2]
        distance_to_goal = torch.linalg.norm(self._desired_pos_w[:, :2] - root_pos_w, dim=1)
        # Обнаружение контакта корпуса (box2_Link или base_link)
        contact_forces = torch.norm(self._contact_sensor.data.net_forces_w, dim=-1)  # [num_envs, 2]
        has_contact = (contact_forces > 0.1).any(dim=-1)  # Контакт на любом из двух примитивов
    ...
    def _reset_idx(self, env_ids: torch.Tensor | None):
        ...
        # Логирование контакта корпуса (box2_Link или base_link)

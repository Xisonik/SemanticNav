...
module_path = os.path.join(current_dir, "source/isaaclab_tasks/isaaclab_tasks/direct/aloha_nav/config_codec.py")
...
class PathGeneratorV2:
    ...
    def __init__(
        ...
        goal_grid_types: Iterable[str] = ("goal_possible_positions",),
...
if __name__ == "__main__":
    pg = PathGeneratorV2(
        config_path="source/isaaclab_tasks/isaaclab_tasks/direct/aloha_nav/scene_items.json",  # твой JSON
        ratio=4, room_len_x=10, room_len_y=10,
        shift_xy=(5, 5),
        robot_radius=0.5,
        room_bounds={"x_min": -5, "x_max": 5, "y_min": -5, "y_max": 5},
        obstacle_types=("movable_obstacle",),   # можно менять "static_obstacle"
        goal_grid_types=("goal_possible_positions",),                    # сетки для целей
        max_movable_subset_size=4,        # ограничь мощность комбинаций movable
        min_movable_subset_size=0,        # ограничь мощность комбинаций movable
        limit_start_nodes=None,           # можно ограничить число стартов на таргет
        max_start_obstacle_dist=10, # метры

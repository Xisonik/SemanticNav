...
class SceneManager:
    def __init__(self, num_envs: int, config_path: str, device: str):
        ...
        self.codebook = self._load_codebook(f"{os.getcwd()}/source/isaaclab_tasks/isaaclab_tasks/direct/aloha_nav/cdecode_dict.json")
    ...
    # ----------- Рандомизация сцены -----------
    def randomize_scene(self, config, env_ids: torch.Tensor, mess: bool = False, use_obstacles: bool = False, use_staff: bool = False, all_defoult: bool = True):
        device = self.device
        self.active[env_ids] = False
        self.positions[env_ids] = self.default_positions[env_ids]
        self.on_surface_idx[env_ids] = -1
        ...
        if all_defoult:
            self.graph.refresh()
            return
        # print(f"[DEBUG RANDOMIZE] env_ids: {env_ids.tolist()}")
        ...
        def _apply_strategy_one_env(p_type: str, env_id: int, env_ids: torch.Tensor, obj_idx_row: torch.Tensor, config, mess=False):
            ...
            type_strats[p_type].apply(env_row, env_ids, obj_idx_row.to(torch.long), scene_data, config, mess)
        ...
        for env_id in env_ids.tolist():
            ...
            for p_type in placement_order:
                if p_type == "surface_provider":
                    ...
                    _apply_strategy_one_env("surface_provider", env_id, env_ids, picked, config)
                    ...
                    _apply_strategy_one_env("movable_obstacle", env_id, env_ids, picked, config)
                    ...
                    _apply_strategy_one_env("staff_obstacle", env_id, env_ids, picked, config)
    ...
    def encode_scene_graph(
        ...
        # 1) Числовые фичи
        obs = self.get_graph_obs(env_ids)  # node_features: [E,M,14], edge_features: [E,M,6]
        node_feats: torch.Tensor = obs["node_features"]  # [E, M, 14]
        edge_feats: torch.Tensor = obs["edge_features"]  # [E, M, 6]

        # 2) Кодбук
        name_map = self.codebook.get("names", {})
        color_map = self.codebook.get("colors", {})

        name_idx = torch.zeros((M, 1), dtype=torch.float32, device=device)
        color_idx = torch.zeros((M, 1), dtype=torch.float32, device=device)

        for j in range(M):
            base_name = self.names[j].split("_", 1)[0].lower()
            name_idx[j, 0] = float(int(name_map.get(base_name, 0)))

            cname = ColorQuantizer.rgb_to_name(self.colors[0, j])
            color_idx[j, 0] = float(int(color_map.get(cname, 0)))
    ...
    def reorder_by_goal(self, per_object_feats: torch.Tensor, goal_idxs: torch.Tensor) -> torch.Tensor:
        """
        per_object_feats: [E, M, D]
        goal_idxs:        [E]  (индекс goal-объекта для каждого env)
        return:           [E, M, D]  где goal всегда на позиции 0
        """
        device = per_object_feats.device
        E, M, D = per_object_feats.shape

        goal_idxs = goal_idxs.to(device).long().clamp(0, M - 1)          # [E]
        base = torch.arange(M, device=device).view(1, M).expand(E, M)    # [E, M]
        order = (base + goal_idxs.view(E, 1)) % M                        # [E, M]

        return per_object_feats.gather(
            dim=1,
    ...
    def _load_codebook(self, codebook_path: str):
        ...
        {
            "names":  { "<имя>": "код_строкой", ... },
            "colors": { "<цвет>": [c1, c2, c3], ... }
        }
        Кэширует его, чтобы не читать файл каждый раз.
        """
        if not hasattr(self, "_codebook_cache") or getattr(self, "_codebook_cache_path", None) != codebook_path:
            with open(codebook_path, "r", encoding="utf-8") as f:
                self._codebook_cache = json.load(f)
            self._codebook_cache_path = codebook_path

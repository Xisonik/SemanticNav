...
class WheeledRobotEnvCfg(DirectRLEnvCfg):
    ...
    action_space = gym.spaces.Box(
        ...
        shape=(2,))
    # observation_space = 7
    observation_space = gym.spaces.Box(low=-float("inf"), high=float("inf"), shape=(5,), dtype="float32")
    state_space = 0
    debug_vis = True
...
class WheeledRobotEnv(DirectRLEnv):
    ...
    def _get_observations(self) -> dict:
        ...
        if self.step_counter > 100  and False:
            ...
            self.step_counter = 0
            # self.use_controller = False if self.use_controller else True
    ...
    def _get_rewards(self) -> torch.Tensor:
        ...
        reward = (base_penalty + goal_reached_reward)
    ...
    def get_contact(self):
        ...
        if force_matrix is not None and force_matrix.numel() > 0:
            ...
            high_contact_envs = num_contacts_per_env > 15
    ...
    def _get_dones(self) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        died = torch.logical_or(
            self.goal_reached(distance_to_goal),
            has_contact
        )
    ...
    def _reset_idx(self, env_ids: torch.Tensor | None):
        ...
        robot_pos, quaternion, goal_pos = self.control_manager.reset(env_ids, self._terrain.env_origins)
        self._desired_pos_w[:, :2] = goal_pos

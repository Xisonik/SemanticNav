...
class OrientationModule(nn.Module):
    ...
    def __init__(self, img_dim: int, graph_emb_dim: int, num_bins: int = 36, 
        ...
        # Убираем embedding_proj, так как будем использовать probs напрямую
...
class StochasticActor(GaussianMixin, Model):
    ...
    def __init__(self, observation_space, action_space, device, shared_graph: SharedGraphModule,
        ...
        mlp_in = self.img_dim + GRAPH_EMB_DIM + ORIENTATION_PROBS_DIM
        self.net = nn.Sequential(
    ...
    def compute(self, inputs, role):
        ...
        x = torch.cat([img, graph_emb, orientation_probs], dim=-1)
        mu = self.net(x)
...
class Critic(DeterministicMixin, Model):
    ...
    def __init__(self, observation_space, action_space, device, shared_graph: SharedGraphModule,
        ...
        mlp_in = self.img_dim + GRAPH_EMB_DIM + ORIENTATION_PROBS_DIM + self.num_actions
        self.net = nn.Sequential(
            nn.Linear(mlp_in, 512),
            nn.ReLU(),
            nn.Linear(512, 256),
    ...
    def compute(self, inputs, role):
        ...
        x = torch.cat([img, graph_emb, orientation_probs, actions], dim=-1)
        q = self.net(x)
...
# 2. Загружаем веса из чекпоинта для ОБОИХ модулей
...
# Извлекаем веса orientation_module из critic_1
orientation_weights = {}
graph_weights = {}
...
if not graph_weights:
    print(f"⚠️  WARNING: No shared_graph weights found in {ORIENTATION_CHECKPOINT}")
    print("   Using randomly initialized shared_graph")
else:
    # Загружаем веса shared_graph
    shared_graph.load_state_dict(graph_weights)
    print(f"✅ Loaded shared_graph from {ORIENTATION_CHECKPOINT}")
    print(f"   Loaded {len(graph_weights)} weight tensors")

# Загружаем веса orientation_module
...
print(f"✅ Both modules frozen (eval mode, no gradients)")

# ---------------------------------------------------------------------
# Models - ВСЕ модели используют frozen модули
# ---------------------------------------------------------------------
models = {
    ...
    "critic_1": Critic(
        ...
        shared_graph=shared_graph,
        orientation_module=orientation_module,
        train_graph=False,      # НЕ обучаем граф
        train_orientation=False # НЕ обучаем ориентацию
    ),
...
if DEBUG:
    print(f"\n{'='*60}")
    print("PARAMETER CHECK (WITH FROZEN MODULES)")
    print(f"{'='*60}")
    
    print("\n1. Shared graph parameters (FROZEN):")
    graph_params = list(shared_graph.parameters())
    print(f"   Total: {sum(p.numel() for p in graph_params):,} parameters")
    print(f"   All requires_grad=False: {all(not p.requires_grad for p in graph_params)}")
    ...
    print("\n3. Critic_1 parameters (should NOT include frozen modules):")
    critic1_params = list(models["critic_1"].parameters())
    print(f"   Total critic net params: {sum(p.numel() for p in critic1_params):,} parameters")
    
    # Проверяем что frozen параметры НЕ в trainable параметрах critic_1
    critic1_trainable_ids = {id(p) for p in critic1_params if p.requires_grad}
    frozen_param_ids = {id(p) for p in graph_params + orient_params}
    frozen_in_critic1_trainable = bool(critic1_trainable_ids & frozen_param_ids)
    
    print(f"\n   ✓ Frozen params in Critic_1 trainable params: {frozen_in_critic1_trainable}")
    if frozen_in_critic1_trainable:
        print(f"   ⚠️  WARNING: Frozen params found in trainable params!")
    else:
        print(f"   ✅ Frozen correctly excluded from training")
...
cfg["experiment"]["directory"] = "logs/skrl/aloha_ppo_orientation_frozen"
...
# 1. Проверяем параметры frozen модулей
frozen_params = list(shared_graph.parameters()) + list(orientation_module.parameters())
print(f"\nFrozen modules:")
print(f"  Total params: {sum(p.numel() for p in frozen_params):,}")
print(f"  All requires_grad=False: {all(not p.requires_grad for p in frozen_params)}")

# 2. Проверяем что frozen параметры НЕ в оптимизаторе
optimizer_params = []
...
frozen_in_optimizer = {id(p) for p in frozen_params} & optimizer_param_ids

print(f"\nOptimizer check:")
print(f"  Frozen params in critic_optimizer: {len(frozen_in_optimizer)}/{len(frozen_params)}")
if len(frozen_in_optimizer) == 0:
    print(f"  ✅ Frozen params correctly NOT in optimizer")
else:
    print(f"  ❌ ERROR: {len(frozen_in_optimizer)} frozen params in optimizer!")

# 3. Проверяем что в оптимизаторе только параметры сетей актора и критика
print(f"\nOptimizer structure:")

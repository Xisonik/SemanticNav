...
class StochasticActor(GaussianMixin, Model):
    def __init__(self, observation_space, action_space, device,
        ...
        mlp_in = img_dim + GRAPH_EMB_DIM + 1  # img + graph_emb + pred_angle
    ...
    def compute(self, inputs, role):
        ...
        x = torch.cat([img, graph_emb, pred_angle], dim=-1)
...
class Critic(DeterministicMixin, Model):
    def __init__(self, observation_space, action_space, device,
        ...
        img_dim = int(observation_space["img"].shape[0])
        mlp_in = img_dim + GRAPH_EMB_DIM + 1 + self.num_actions
    ...
    def compute(self, inputs, role):
        ...
        x = torch.cat([img, graph_emb, pred_angle, actions], dim=-1)
        return self.net(x), {}

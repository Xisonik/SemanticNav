...
class OrientationModule(nn.Module):
    ...
    def forward(self, img: torch.Tensor, graph_emb: torch.Tensor, 
        ...
        if ground_truth_yaw is not None:
            ...
            # Дискретизация для accuracy
            normalized = (ground_truth_yaw + torch.pi) % (2 * torch.pi)
            labels = (normalized / bin_size).long().clamp(0, self.num_bins - 1)
            
            # ✅ CIRCULAR LOSS вместо cross-entropy
            probs = F.softmax(logits, dim=-1)  # [B, num_bins]
            
            # Ожидаемый угол из распределения
            bin_centers = torch.linspace(0, 2*torch.pi, self.num_bins+1, device=logits.device)[:-1]
            bin_centers = bin_centers + bin_size / 2  # Центры бинов
            
            # Взвешенная сумма углов (circular mean)
            pred_angle_sin = torch.sum(probs * torch.sin(bin_centers), dim=-1)
            pred_angle_cos = torch.sum(probs * torch.cos(bin_centers), dim=-1)
            pred_angle = torch.atan2(pred_angle_sin, pred_angle_cos)
            
            # Circular distance
            angle_diff = torch.atan2(
                torch.sin(normalized - pred_angle),
                torch.cos(normalized - pred_angle)
            )
            loss = torch.mean(angle_diff ** 2)  # MSE в circular space
            
            # Accuracy (допуск ±1 бин)
            pred_bins = torch.argmax(logits, dim=-1)
            bin_diff = torch.abs(pred_bins - labels)
            bin_diff = torch.minimum(bin_diff, self.num_bins - bin_diff)  # Circular
            accuracy = (bin_diff <= 1).float().mean()
    ...
    #         # ✅ ИСПРАВЛЕННАЯ LOSS с нормализацией
    #         # Circular distance в радианах
    #         angle_diff = torch.atan2(
    #             torch.sin(gt_normalized - pred_angle),
    #             torch.cos(gt_normalized - pred_angle)
    #         )
            
    #         # Масштабируем в [0, 1]: 0° → 0, 180° → 1
    #         normalized_error = 100 * torch.abs(angle_diff) # / torch.pi
            
    #         # MSE в normalized space
...
if not EVAL:
    ...
    checkpoint_path = "/home/xiso/IsaacLab/logs/skrl/aloha_ppo_orientation/26-02-02_15-52-04-276625_SAC/checkpoints/agent_2500.pt"
